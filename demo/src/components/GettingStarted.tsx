...
const STEPS = [
  {
    ...
    description:
      "Add envage as a dev dependency in your monorepo root. Works with npm, yarn, and pnpm.",
...
yarn add --dev @asafarim/envage`,
  ...
  },
  {
    ...
    description:
      "Run init-key once in your repo root. It creates a private key (.gitignored automatically) and a public key you can share with teammates.",
...
# ⚠ IMPORTANT: Never commit your private key (.age/key.txt).`,
  ...
  },
  {
    ...
    description:
      'Create envage.config.json at your monorepo root. List your app folders and environment names. Entries support glob patterns (apps/*, packages/**) that expand to matching folders. Use "root" as an env name to target a plain .env file instead of .env.<env>.',
...
// Glob patterns expand to every matching folder:
{
  "apps": [".", "apps/*", "packages/*"],
  "envs": ["root", "production"],
  "keyFile": ".age/key.txt"
}
// "apps/*" → apps/web, apps/admin, ... (existing dirs only)
...
// "root" → encrypts .env → .env.age`,
  ...
  },
  {
    ...
    description:
      "Create your .env files normally, then encrypt them. Only the .age files go into Git — the plaintext files are gitignored.",
            ...
            {STEPS.map((s, i) => (
              <button
                ...
                style={{
                  ...
                  padding: "16px 20px",
                  background:
                    activeStep === i ? "var(--bg-card)" : "transparent",
                  border:
                    activeStep === i
                      ? `1px solid ${s.color}`
              ...
              >
                <span
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      activeStep === i ? s.color : "var(--bg-tertiary)",
