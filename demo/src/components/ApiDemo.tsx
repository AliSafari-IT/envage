...
//          .age/key.pub`,
  },
  {
    label: "Status",
    icon: "📊",
    code: `import { getEnvStatus, loadConfig } from "@asafarim/envage";
...
function highlight(code: string) {
  return code
    .replace(/(\/\/.+)/g, `<span style="color:var(--terminal-comment)">$1</span>`)
    .replace(/("([^"]*)")/g, `<span style="color:var(--terminal-string)">$1</span>`)
    .replace(/\b(import|from|await|const|async|if|process)\b/g, `<span style="color:var(--terminal-keyword)">$1</span>`)
    .replace(/\b(true|false|null|undefined)\b/g, `<span style="color:var(--accent-orange)">$1</span>`)
    .replace(/\b(\d+)\b/g, `<span style="color:var(--accent-purple)">$1</span>`);
}
...
export default function ApiDemo() {
  ...
  return (
    <section id="api-demo" className="section" style={{ background: "var(--bg-secondary)" }}>
      <div className="container">
        <p className="section-label">Programmatic API</p>
        <h2 className="section-title">Use it as a library</h2>
        <p className="section-subtitle" style={{ marginBottom: 48 }}>
          Every CLI command has an equivalent TypeScript function. Import what you
          need and integrate it into your build scripts, CI pipelines, or custom tooling.
        </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {SNIPPETS.map((s, i) => (
              <button key={s.label} onClick={() => setActive(i)} style={{
                ...
                border: "1px solid",
                borderColor: active === i ? "var(--accent-blue)" : "transparent",
            ...
            {/* TypeScript badge */}
            <div style={{ marginTop: 16, padding: "12px 14px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--accent-blue)", marginBottom: 6 }}>
                🔷 Fully typed
              </div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
                All exports ship with TypeScript declarations. IntelliSense works out of the box.
              </div>
            </div>
          </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: "0.9rem" }}>{SNIPPETS[active].icon}</span>
              ...
              <pre style={{
                ...
                dangerouslySetInnerHTML={{ __html: highlight(SNIPPETS[active].code) }}
              />
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 48 }}>
          <h3 style={{ marginBottom: 20, color: "var(--text-primary)" }}>Type exports</h3>
          <div className="grid-3">
            {[
              { name: "EnvOptions", desc: "Options for encryptEnv / decryptEnv (folder, env, keyFile, passphrase)" },
              { name: "EnvStatus", desc: "Status of a single folder+env slot: encrypted, decrypted, paths" },
              { name: "EnvageConfig", desc: "Shape of envage.config.json: apps, envs, keyFile, keyPubFile" },
            ].map((t) => (
              <div key={t.name} className="card" style={{ padding: "20px 24px" }}>
                <code style={{ color: "var(--accent-blue)", fontSize: "0.9rem", fontWeight: 600 }}>
                  {t.name}
                </code>
